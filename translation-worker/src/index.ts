...
const SEO_DESCRIPTION_FALLBACK_SUFFIX = 'Capacitor Tutorial helps developers learn Capacitor, build native apps, and ship updates with practical guides and examples.'
...
async function fetchEnglishOrigin(request: Request, env: Env, requestUrl: URL): Promise<Response> {
  ...
  return await env.WEB.fetch(createOriginRequest(request, originUrl))
}
...
function testProbePathParam(requestUrl: URL): string {
  ...
  const pathUrl = new URL(rawPath, 'https://capacitor-tutorial.com')
...
async function probeRealPageTranslation(env: Env, requestUrl: URL): Promise<Record<string, unknown>> {
  ...
  const sourceUrl = new URL(path, 'https://capacitor-tutorial.com')
