...
const DEVELOPMENT_R2_BUCKET = 'capacitor-tutorial-translation-cache-development'
...
const REAL_PAGE_PROBES = ['/', '/blog/'] as const
...
async function ensureDevelopmentBucket(): Promise<void> {
  ...
  if (result.code === 0 || /already exists|already.*capacitor-tutorial-translation-cache-development/i.test(result.output)) return
