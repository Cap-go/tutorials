...
```ts
import { FacebookLogin, FacebookLoginResponse } from '@whiteguru/capacitor-plugin-facebook-login'
...
```ts
import { FacebookLogin, FacebookLimitedLoginResponse } from '@whiteguru/capacitor-plugin-facebook-login'

const FACEBOOK_PERMISSIONS = ['email', 'user_birthday', 'user_photos', 'user_gender']
const result = await (<FacebookLimitedLoginResponse>FacebookLogin.loginWithLimitedTracking({ permissions: FACEBOOK_PERMISSIONS }))
