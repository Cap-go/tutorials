...
<p align="center"><strong><code>@leadscout/intercom</code></strong></p>
<p align="center">
...
This is a forked repo from [@capacitor-community/intercom](https://www.npmjs.com/package/@capacitor-community/intercom).

This fork adds a few updates:
1. Add `isUserLoggedIn` API.
2. Allow `company` updates in the `updateUser` API.
3. iOS: Proper use of DispatchQueue for various iOS Intercom API calls.
4. Android:
   1. Update deprecated "register..." function usage (e.g. `Intercom.client().registerIdentifiedUser()`).
   2. Update deprecated "display..." function usage (e.g. `Intercom.client().displayMessenger()`)

## Maintainers (from [@capacitor-community/intercom](https://www.npmjs.com/package/@capacitor-community/intercom))
...
| **`company`**          | <code>{ companyId: string; name: string; createdAt?: number; customAttributes?: { [key: string]: any; }; }</code> |
